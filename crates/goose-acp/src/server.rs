...
impl GooseAcpAgent {
    ...
    async fn on_new_session(
        ...
        // Use the internal session ID for agent creation (it keys into the sessions DB).
        let agent_session_id = SessionId::new(internal_session_id.clone());

        let agent = self
            .create_agent_for_session(Some(cx), Some(&agent_session_id), None)
            .await
            .map_err(|e| {
        ...
        Self::add_mcp_extensions(&agent, args.mcp_servers, &internal_session_id).await?;
        ...
        info!(
            ...
            "ACP session started (thread created)"
        );
    ...
    async fn on_load_session(
        ...
        agent
            .update_goose_mode(loaded_mode, &internal_session_id)
            .await
            .map_err(|e| {
        ...
        info!(
            thread_id = %thread_id,
            internal_session_id = %internal_session_id,
            goose_mode = %loaded_mode,
            "ACP session loaded (thread resumed)"
        );
